...
- [x] **PDF 텍스트 추출 시스템 완성** (6가지 방법을 통한 강력한 텍스트 추출)
- [x] **텍스트 품질 검증 시스템** (바이너리 데이터 필터링, 학술 콘텐츠 인식)
- [x] **다중 페이지 선택 옵션** (첫 페이지/현재 페이지/범위 지원)
- [x] **OpenAI API 통신 모듈 완성** (`ai/openai.js`)
- [x] **Ollama API 통신 모듈 완성** (`ai/ollama.js`)
- [x] **통합 설정 시스템 구현** (`config/settings.js`, `prefs.xhtml`)

### ❌ 제약사항으로 불가능한 기능
- **이미지 렌더링 기반 OCR**: Zotero 7의 PDF.js 보안 제약으로 인해 page 객체의 render 메서드 접근 불가
- **Canvas 요소 접근**: PDF 뷰어 DOM에서 렌더링된 canvas 요소가 플러그인에서 접근 불가능
- **강제 PDF 렌더링**: 내장 PDF.js의 제한된 API로 인해 이미지 생성 불가

### 🚧 진행 중인 작업 (8단계 - 설정 시스템)
- **설정 UI와 bootstrap.js 완전 연동** - 설정 변경 시 실시간 반영
- **실제 API 연결 테스트** - 형식 검증을 넘어 실제 API 호출 테스트

### 📋 다음 단계 (6-7단계 - 메타데이터 처리 및 Parent 생성)
- [ ] AI 모듈과 PDF 텍스트 추출 시스템 연결
- [ ] AI JSON 응답을 Zotero 필드로 매핑 (title, authors, year, journal, DOI)
- [ ] `Zotero.Items.add()`를 통한 새 parent 생성
- [ ] PDF 첨부파일 연결 및 관계 설정
- [ ] 기존 parent 중복 여부 검사 (DOI, 제목 기준)
...
5. 추출된 텍스트로 AI 호출 준비 완료 (OpenAI/Ollama 모듈 구현됨)
6. **설정 UI**: Zotero 플러그인 매니저에서 Options 버튼으로 접근 가능
