...
### 🚧 **현재 상황**
**단계 8 (설정 시스템) 진행 중** - 기본 구조 완성, 실제 연동 작업 필요
- **설정 UI ↔ bootstrap.js 완전 연동** - 설정 변경 시 실시간 반영
- **실제 API 연결 테스트** - 형식 검증을 넘어 실제 API 호출 테스트
...
### 🔮 **향후 작업**
- [x] **End-to-End 워크플로우 완성** - PDF 텍스트 → AI 호출 → parent 생성 ✅ **완료**
- [ ] 예외 처리 및 에러 핸들링 강화 (9단계)
- [ ] 사용자 경험 개선 (10단계)
...
### 현재 기능 동작 (End-to-End 워크플로우 완성)
1. PDF 파일 열기 시 자동으로 RefSense 버튼 추가
2. 버튼 클릭 또는 Ctrl+Shift+E 입력
3. **텍스트 레이어 있는 PDF**: 완벽한 텍스트 추출 및 품질 검증
4. **텍스트 레이어 없는 PDF**: "이미지 추출 불가" 메시지 표시
5. **AI 호출 및 메타데이터 추출**: OpenAI/Ollama를 통한 서지정보 분석
6. **JSON 파싱 및 데이터 매핑**: AI 응답을 Zotero 필드로 변환
7. **기존 Parent 검사**: 이미 parent가 있는 경우 사용자 선택 대화상자
   - **3가지 옵션**: "기존 정보 업데이트", "새 Parent 생성", "취소"
8. **필드별 선택적 업데이트** (업데이트 선택 시):
   - **비교 대화상자**: 기존 vs 새 정보 나란히 표시
   - **개별 필드 선택**: 라디오 버튼으로 원하는 값 선택
   - **편의 기능**: "모두 기존 값", "모두 새 값" 일괄 선택
   - **시각적 피드백**: 선택된 행 색상 변경, 툴팁 지원
9. **Parent Item 처리**: 새 생성 또는 선택적 업데이트 수행
10. **PDF 연결**: parent-child 관계 설정 및 완료 알림
11. **설정 UI**: Zotero 플러그인 매니저에서 Options 버튼으로 접근 가능
